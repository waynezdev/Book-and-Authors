...
async function index (req, res) {
  ...
  res.render('books/index', { books }) // Render the 'author/index view' pass it the books
}
...
async function show (req, res) {
  ...
  let author = await BookModel.findById(id) // Find the author by id and add save it to the variable author
...
async function edit (req, res) {
  let { id } = req.params // Destructure off the id off req.params
  let author = await BookModel.findById(id) // query the DB for the author by ID
  res.render('author/edit', { author }) // Pass that author through to 'books/edit'
}
...
async function update (req, res) {
  ...
  console.log(req.params)
  let { id } = req.params // Destructure off the id off req.params
  await BookModel.findByIdAndUpdate(id, { name, bio, gender }) // Update the author. Find by the id and then update with the new name, bio and gender
  res.redirect(`/books/${id}`) // Redirect to the show page for author we just updated
}
